document.addEventListener('DOMContentLoaded', function() {
  ...
  var serviceEls = document.querySelectorAll('.service-card');
  var extraEls = document.querySelectorAll('.testimonial-card, .footer-col');
  ...
  var extraObserver = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      if (entry.isIntersecting) {
        entry.target.classList.add('revealed');
  ...
  serviceEls.forEach(function(el) { serviceObserver.observe(el); });
  extraEls.forEach(function(el) { extraObserver.observe(el); });

  // ======== GSAP: About image parallax zoom ========
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    gsap.registerPlugin(ScrollTrigger);
    var aboutImg = document.querySelector('.about-image img');
    if (aboutImg) {
      gsap.fromTo(aboutImg, { scale: 0.9, opacity: 0.6 },
